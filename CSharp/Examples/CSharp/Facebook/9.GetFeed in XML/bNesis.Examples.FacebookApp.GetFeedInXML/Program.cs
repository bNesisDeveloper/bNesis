...
namespace bNesis.Examples.FacebookApp.GetFeedInXML
{
    /// <summary>
    /// This console example shows how to use bNesis SDK initialization, authentication and to get 
    /// the feed of posts (including status updates) and links published by this person, or by others on this person's profile.
    /// All data will be wroten to XML file to a folder \"container\" in the application folder.
    ...
    internal class Program
    {
        ...
        /// <summary>
        /// If you use a Thin Client mode, you need an access to one of bNesis API Servers. Addresses of the demo bNesis API servers:
        ...
        /// <summary>
        /// If you use the Thin Client mode, you need an access to bNesis API Server. Addresses of the available demo bNesis API Servers see above
        /// </summary>
        ...
        static void Main(string[] args)
        {
            ...
            //Waiting for a pressed key. If key is not 'R', you will use Thin client as a default mode.
            ...
            else
            {
                ...
                try
                {
                    ...
                    //Getting the feed of posts
                    Response feedPosts = facebook.GetUserFeedRaw("me");
                    ...
                    {
                        //Show error message
                        Console.WriteLine("Getting the feed of posts has Failed, error code:" + errorInfo.Code +
                    ...
                    Console.WriteLine("Success! The feed of posts is received");

                    Console.WriteLine("Saving of data to an XML file...");

                    //Convert JsonString Response to XML document
                    XDocument xml = XDocument.Load(JsonReaderWriterFactory.CreateJsonReader(
                        Encoding.ASCII.GetBytes(feedPosts.Content), new XmlDictionaryReaderQuotas()));

                    //Writing xml document to the file
                    Directory.CreateDirectory("container");
                    xml.Save("container/data.xml", SaveOptions.None);

                    Console.WriteLine("A feed of the post is saved to a folder \"container\" in the application folder.");
