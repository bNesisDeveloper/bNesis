...
/// </summary> 
namespace bNesis.Examples.FacebookApp.GetCoverInXML
{
    /// <summary>
    /// This console example shows how to use bNesis SDK initialization, authentication and to get the person's cover photo.
    /// Use without scopes.
    /// All data will be wroten to the XML file to the folder "container" in the application folder.
    /// Photo is saved in the folder "container/photo" in a application folder.
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    //Getting the person's cover photo
                    Response cover = facebook.GetUserCoverRaw("me");
                    ...
                    {
                        //Show error message
                        Console.WriteLine("Getting the person's cover photo is Failed, error code:" + errorInfo.Code +
                    ...
                    Console.WriteLine("Success! The person's cover photo is received.\n");
                    ...
                    XDocument xml = XDocument.Load(JsonReaderWriterFactory.CreateJsonReader(
                        Encoding.ASCII.GetBytes(cover.Content), new XmlDictionaryReaderQuotas()));

                    //Writing xml document to the file in 
                    Directory.CreateDirectory("container");
                    xml.Save("container/data.xml", SaveOptions.None);

                    Console.WriteLine("The person's cover photo is saving to the folder \"container\" in the application folder.\n");

                    //Downloading photo
                    Console.WriteLine("Photo downloading...");

                    //Convert a Response to the class FacebookUserPhotos
                    FacebookUser coverPhoto = JsonConvert.DeserializeObject<FacebookUser>(cover.Content);

                    //Downloading photos
                    string link = coverPhoto.cover.source;
                    Directory.CreateDirectory("container/photo");
                    WebClient webClient = new WebClient();
                    webClient.DownloadFile(link, $"container/photo/cover.jpg");

                    Console.WriteLine($"Downloading is completed. Photo is saved in the folder \"container\\photo\" in a application folder.\n");
