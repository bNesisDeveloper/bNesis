...
/// </summary> 
namespace bNesis.Examples.FacebookApp.GetFriends
{
    /// <summary>
    /// This console example shows how to use bNesis SDK initialization, authentication and to get the user's friends with scope "user_friends"
    ...
    internal class Program
    {
        ...
        /// </summary>
        private static string[] Scope = new string[] {"user_friends"};

        /// <summary>
        /// If you use the Thin Client mode, you need an access to the bNesis API Server. Address of the demo bNesis API server:
        ...
        /// 
        /// In Thin client mode add two redirectURIs to tab "Authentication" at Application Settings. The first redirectURI relates what bNesis API server you use:
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    FacebookUserFriends listFriends = facebook.GetFieldsUserFriends(id);
                    ...
                    {
                        //Show error message
                        Console.WriteLine("Getting friends is Failed, error code:" + errorInfo.Code +
                    ...
                    Console.WriteLine("Success! A list of friends is received. Totaly {0} friends.", listFriends.summary.total_count);


                    //Information about each friend who allowed this application. 
                    Console.WriteLine("\nList of friends who allowed this application:");

                    for (int i=0; i<listFriends.data.Length; i++)
                    {
                        string friendInfo = string.Empty;

                        if (!string.IsNullOrEmpty(listFriends.data[i].name))
                        {
                            friendInfo = " Name: " + listFriends.data[i].name;
                        }

                        if(!string.IsNullOrEmpty(listFriends.data[i].birthday))
                        {
                            friendInfo = friendInfo + ", birth date: " + listFriends.data[i].birthday;
                        }

                        if(!string.IsNullOrEmpty(listFriends.data[i].gender))
                        {
                            friendInfo = friendInfo + ", gender: " + listFriends.data[i].gender;
                        }

                        Console.WriteLine(i+1 +"."+ friendInfo);
                    }
