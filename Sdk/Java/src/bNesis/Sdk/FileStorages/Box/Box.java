package bNesis.Sdk.FileStorages.Box;
...
import java.net.URI.*;
	...
	public class Box  
	{
		...
		public Box(bNesisApi bnesisApi)
		{
		...
		{
			bNesisToken = _bNesisApi.Auth("Box", "",bNesisDevId,redirectUrl,clientId,clientSecret,null,"","",false,"");
			return bNesisToken;
		}
		...
		{
			Boolean result = _bNesisApi.LogoffService("Box", bNesisToken);
		...
		{
			return _bNesisApi.<ErrorInfo>Call(ErrorInfo.class, "Box", bNesisToken, "GetLastError");
		}
		...
		{
			return _bNesisApi.<ErrorInfo[]>Call(ErrorInfo[].class, "Box", bNesisToken, "GetErrorLog");
		}
		...
		{
			return _bNesisApi.<Boolean>Call(Boolean.class, "Box", bNesisToken, "ValidateToken");
		}
	 ...
	 * if token doesn't support token revocation or token is revoked method returns false 
		...
		{
			return _bNesisApi.<Boolean>Call(Boolean.class, "Box", bNesisToken, "Logoff");
		}
	 ...
	 * @return {String} String with normalized path. 
		...
		{
			return _bNesisApi.<String>Call(String.class, "Box", bNesisToken, "NormalizePath", path);
		}
	 ...
	 * @return {FileStorageItem[]} Array of IFileStorageItem. 
		...
		{
			return _bNesisApi.<FileStorageItem[]>Call(FileStorageItem[].class, "Box", bNesisToken, "GetFiles", path);
		}
	 ...
	 * @return {FileStorageItem[]} Array of IFileStorageItem. 
		...
		{
			return _bNesisApi.<FileStorageItem[]>Call(FileStorageItem[].class, "Box", bNesisToken, "GetFolders", path);
		}
	 ...
	 * @return {String} Empty string if OK or an error description otherwise. 
		...
		{
			return _bNesisApi.<String>Call(String.class, "Box", bNesisToken, "CreateFolder", path, name);
		}
	 ...
	 * @return {String} Empty string if OK or an error description otherwise. 
		...
		{
			return _bNesisApi.<String>Call(String.class, "Box", bNesisToken, "Delete", path);
		}
	 ...
	 * @return {OutputStream} Stream with file or null if error. 
		...
		{
			return _bNesisApi.<OutputStream>Call(OutputStream.class, "Box", bNesisToken, "DownloadFile", path);
		}
	 ...
	 * @return {String} Empty string if OK or an error description otherwise. 
		...
		{
			return _bNesisApi.<String>Call(String.class, "Box", bNesisToken, "UploadFile", destinationStream, name);
		}
		 ...
		 * @throws Exception
		 */
		public Response GetCurrentUserRaw() throws Exception
		{
			return _bNesisApi.<Response>Call(Response.class, "Box", bNesisToken, "GetCurrentUserRaw");
		}
}